...
export const GET = jsonRoute(async () => {
  ...
  const [availability, memory, languages] = await Promise.all([
    getAvailability(session.user.id),
    memorySnapshot(session.user.id),
    getLanguages(session.user.id),
  ]);
  return NextResponse.json({ availability, memory, languages });
});
...
export const PATCH = jsonRoute(async (req: Request) => {
  ...
  const { languages, ...availability } = parsed.data;

  if (languages) await setLanguages(session.user.id, languages);

  const [availabilityResult, languagesResult] = await Promise.all([
  ...
  return NextResponse.json({ availability: availabilityResult, languages: languagesResult });
});
